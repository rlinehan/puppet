...
class Puppet::Indirector::REST < Puppet::Indirector::Terminus
  ...
  def head(request)
    response = do_request(request) do |request|
      http_head(request, IndirectedRoutes.request_to_uri(request), headers)
    end
  ...
  def search(request)
    response = do_request(request) do |request|
      http_get(request, IndirectedRoutes.request_to_uri(request), headers)
    end
  ...
  def destroy(request)
    ...
    response = do_request(request) do |request|
      http_delete(request, IndirectedRoutes.request_to_uri(request), headers)
    end
  ...
  def save(request)
    ...
    response = do_request(request) do |request|
      http_put(request, IndirectedRoutes.request_to_uri(request), request.instance.render, headers.merge({ "Content-Type" => request.instance.mime }))
    end
